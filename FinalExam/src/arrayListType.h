...
public:
    ...
    bool isFull();
      ...
      //    otherwise, returns false.
    void insertionSort();
    // Function that sorts through the list in numerical order
    int listSize();
    ...
    bool isItemAtEqual(int location, const elemType& item);
      ...
      //               returns false.
   void quickSort();
   // calls recQuickSort passing 0, and length -1 ;
   ...
   void insertEnd(const elemType& insertItem);
    ...
    void selectionSort();
	//Function to call the minLocation function passing
    //location and length - 1. Determines minimum index.
	//Postcondition: Calls swap - Lists are swapped.  
...
protected:
    ...
    void recQuickSort(int first, int last); // recursion version of quicksort
    void swap(int first, int second); // Swap the two lists
    int minLocation(int first, int last); //Find minimum index
};
...
firstOutOfOrder++)
if (list
[firstOutOfOrder] < list[firstOutOfOrder - 1])
{
...
void arrayListType<elemType>::recQuickSort(int first, int last)
{
	...
	if (first < last)
	{
		pivotLocation = partition(first, last); // need partition to make a swap
		recQuickSort(first, pivotLocation - 1);
		recQuickSort(pivotLocation + 1, last);
	}
}

// calls recQuickSort() passes it 0, length -1
template <class elemType>
